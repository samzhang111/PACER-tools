...
MAX_LONGTIME_ATTEMPTS = 3
PSC_TEXT = "PACER Service Center"

PAUSE = {"micro": 0.1, "mini": 0.5, "second": 1, "moment": 5, "medium": 10, "mega": 30}
...
def get_xml_response(browser, request_url, request_type="GET"):
    ...
    if resp.status_code != 200:
        ...
        content = json.loads(
            json.dumps(xmltodict.parse(resp.content)).replace('"@', '"')
        )
...
class CoreScraper:
    ...
    def launch_browser(self):
        ...
        options = stools.get_firefox_options(
            self.dir.temp_subdir(self.ind), self.headless
        )
    ...
    def get_caseno_info(self, case_no):
        ...
        if response in ("error", "not_logged_in"):
            ...
            data = [
                {"ucid": dtools.ucid(self.court, case_no), **line} for line in cases
            ]
    ...
    def get_caseno_info_id(self, case_no, def_no=None):
        ...
        if response == "success":
            ...
            if len(data) == 1:
                ...
                candidates = list(
                    filter(lambda x: x.get("defendant", None) == query_def_no, data)
                )
                if len(candidates):
...
class QueryScraper(CoreScraper):
    ...
    def results_found(self):
        ...
        return not self.browser.find_element(
            By.CSS_SELECTOR, "#cmecfMainContent h2"
        ).text.startswith("No information was found")
    ...
    def pull_queries(self):
        ...
        if len(self.config_list) > 1:
            logging.info(
                f"Date filed range was greater than one year, search split into {len(self.config_list)} chunks"
            )
        ...
        for i, config_chunk in enumerate(self.config_list):
            ...
            try:
                ...
                query_form = forms.FormFiller(
                    self.browser, template="query", fill_values=config_chunk
                )
                ...
                if self.results_found():
                    ...
                    logging.info(
                        f"No results found for chunk with index:{i}\
...
class DocketScraper(CoreScraper):
    ...
    re_mem = re.compile(
        """<a href=[\\\]{0,1}["']/cgi-bin/DktRpt.pl\?[0-9]{1,10}[\\\]{0,1}['"]>[0-9]\:[0-9][0-9]\-c[vr]-[0-9]{3,10}</a>"""
    )
    ...
    def __init__(
        ...
        self.update_task_path = (
            stools.get_update_task_file(self.docket_input)
        ...
        self.member_list_seen = (
            get_member_cases(core_args["court_dir"])
            if show_member_list == "avoid"
            else []
        )
        ...
        if len(self.browser.find_elements(By.CSS_SELECTOR, "#cmecfMainContent h3")):
            return (
                ...
                in self.browser.find_element(
                    By.CSS_SELECTOR, "#cmecfMainContent h3"
                ).text
            )
        else:
            return False
    ...
    def at_invalid_case(self):
        ...
        return (
            "not a valid case. Please enter a valid value"
            in self.browser.find_element(By.CSS_SELECTOR, "#cmecfMainContent").text[
                :200
            ]
        )
    ...
    def no_docketlines(self):
        ...
        return bool(
            re.search(
                stools.re_no_docket,
                self.browser.find_element(By.CSS_SELECTOR, "#cmecfMainContent").text,
            )
        )
    ...
    def at_longtime(self):
        ...
        try:
            page_text = self.browser.find_element(
                By.CSS_SELECTOR, "#cmecfMainContent"
            ).text[:200]
    ...
    def pull_case(self, case, new_member_list_seen):
        ...
        case_no_input = (
            case["case_no"] + f"-{case['def_no']}"
            if "def_no" in case.keys()
            else case["case_no"]
        )
        fill_values = {
        ...
        if case.get("latest_date"):
            ...
            logging.info(
                f"Getting updated docket for <case:{case['case_no']}> from <date:{fill_values['date_from']}>"
            )
        ...
        if self.show_member_list == "avoid":
            if not (
                case["case_no"] in self.member_list_seen
                or case["case_no"] in new_member_list_seen
            ):
        ...
        task_line = {
            k: case[k]
            for k in ("ucid", "latest_date", "previously_downloaded")
            if k in case
        }
        ...
        if not self.at_docket_report():
            ...
            if self.at_longtime():
                ...
                initially_requested = self.browser.find_elements(
                    By.CSS_SELECTOR, 'input[name="date_from"]'
                )[-1]
                initially_requested.click()
                time.sleep(PAUSE["micro"])

                logging.info(
                    f"{self} LONGTIME: case {case['ucid']} at the 'long time' page, submitting form and pausing to let page load..."
                )
                ...
                if not transaction_table:
                    print(
                        "ERROR: LONGTIME exceeded maximum attempts and page not fully loaded"
                    )
                    return
            else:
                self.browser.execute_script("ProcessForm()")
        ...
        if self.at_invalid_case():
            ...
            hstring_court, hstring_caseno = extract_court_caseno(
            ...
            if not (hstring_court == self.court and hstring_caseno == case["case_no"]):
                print(
                    f"PACER_ERROR_WRONG_CASE ucid={case['ucid']} {hstring_court=} {hstring_caseno=}"
                )
                return PACER_ERROR_WRONG_CASE

            # Save the output by case name
            outpath = ftools.get_expected_path(
                case["ucid"],
                subdir="html",
                pacer_path=self.dir.root.parent,
                def_no=case.get("def_no"),
            )
            ...
            if case.get("previously_downloaded", False):
                ...
                while outpath.exists():
                    ind += 1
                    outpath = self.dir.html / ftools.generate_docket_filename(
                        case["case_no"], case.get("def_no"), ind=ind
                    )
            ...
            if self.show_member_list == "avoid":
                ...
                if found_members:
                    found_case_ids = [
                        x.split("</a>")[0].split(">")[-1] for x in found_members
                    ]
                    found_case_ids = [
                        ftools.clean_case_id(x)
                        for x in set(found_case_ids)
                        if x not in self.member_list_seen
                    ]
...
def check_exists(
    ...
    exists = ftools.get_expected_path(
        ucid=ucid, subdir=subdir, pacer_path=pacer_path
    ).exists()
...
def _pull_summary_(ScraperInstance, case):
    ...
    try:
        query_form = forms.FormFiller(
            self.browser, template="query", fill_values=fill_values
        )
        time.sleep(PAUSE["mini"])
        query_form.fill()
        time.sleep(PAUSE["mini"])
    ...
    a_candidates = [
        a
        for a in self.browser.find_elements(By.CSS_SELECTOR, "td a")
    ...
    with open(outpath, "w+") as wfile:
        # Add the stamp to the bottom of the url as it is written
        wfile.write(
            self.browser.page_source + self.stamp(download_url, pacer_id=pacer_id)
        )
...
def summary_no_data(browser):
    ...
    return (
        "No case data for case"
        in browser.find_element(By.CSS_SELECTOR, "#cmecfMainContent").text[:300]
    )
    ...
    def pull_members(self, case):
        ...
        if self.none_found():
            ...
            if not case_no:
                ...
                if not match:
                    logging.info(
                        f"{self} ERROR: Cannot identify case_no for this case {pacer_id=}"
                    )
                    return
                else:
...
class DocumentScraper(CoreScraper):
    ...
    def get_previously_downloaded_docs(self):
        ...
        return [
            ftools.parse_document_fname(x.name)["doc_id"]
            for x in self.dir.docs.glob("*/*.pdf")
        ]
    ...
    def pull_docs(self, docket):
        ...
        ucid = docket["ucid"]
        fpath = ftools.get_expected_path(
            ucid,
            subdir="html",
            pacer_path=self.dir.root.parent,
        )
        ...
        if wanted_doc_nos:
            logging.info(
                f"{self} Getting the following documents for case {ucid}:\n{wanted_doc_nos}"
            )
        else:
            logging.info(f"{self} Getting all documents for case {ucid}")

        doc_links = stools.get_document_links(
            docket_table, self.get_att, wanted_doc_nos=wanted_doc_nos
        )
        ...
        for doc in doc_links:
            if doc["get_line_doc"] == True:
                ...
                if success == None:
                    logging.info(
                        f"{self} SKIPPING: previously downloaded case:{ucid} doc index: {doc['ind']}"
                    )
                elif success == False:
                    logging.info(
                        f"{self} ERROR: pull_doc failed for case:{ucid} doc index: {doc['ind']}"
                    )
            ...
            for att in doc.get("atts", []):
                try:
                    ...
                    if success == None:
                        logging.info(
                            f"{self} SKIPPING: previously downloaded case:{ucid} doc index: {doc['ind']} and attachment index: {att['ind']}"
                        )
                    elif success == False:
                        logging.info(
                            f"{self} ERROR: pull_doc failed for case:{ucid} doc index: {doc['ind']} and attachment index: {att['ind']}"
                        )
                except:
                    logging.info(
                        f"{self} ERROR: pull_doc failed for case:{ucid} doc index: {doc['ind']} and attachment index: {att['ind']}"
    ...
    def at_document_selection(self):
        ...
        main_content = self.browser.find_elements(
            By.CSS_SELECTOR, "div#cmecfMainContent"
        )[0]
    ...
    def cannot_redisplay(self):
        ...
        if len(tabs) > 1:
            try:
                ...
                if self.browser.find_element(By.CSS_SELECTOR, "body").text.startswith(
                    "Cannot redisplay"
                ):
                    resp = True
    ...
    def at_outside_warning(self):
        ...
        return main_content.text.startswith(
            "Warning: The link to this page may not have originated from within"
        )
    ...
    def pull_doc(self, doc, ucid, att=None, retry_count=0, from_doc_selection=False):
        ...
        if self.at_document_selection():
            try:
                ...
                logging.info(
                    f"{self} ERROR (pull_doc): Could not select from document selection screen ({doc_id})"
                )
                return False
            ...
            transaction_data = {
                k: v
                for k, v in ftools.parse_transaction_history(
                    self.browser.page_source
                ).items()
            ...
            if transaction_data:
                logging.info(
                    f"{self} <case:{doc_id}> Transaction Receipt: {json.dumps(transaction_data)}"
                )
            ...
            logging.info(
                f"{self} ERROR (pull_doc): Do not have permission to access ({doc_id})"
            )
            return False
        ...
        file = stools.get_recent_download(
            self.dir.temp_subdir(self.ind), wait_time=wait_time, time_buffer=20
        )
        if file:
            # Move file
            if fpath.exists():
            ...
            logging.info(
                f"{self} ERROR (pull_doc): Download not found on disk ({doc_id})"
            )
            return False
...
def generate_dockets_list(document_input, core_args, skip_seen=True, all_docs=False):
    ...
    df["exists"] = df.ucid.apply(
        lambda x: ftools.get_expected_path(
            ucid=x, subdir="html", pacer_path=core_args["court_dir"].root.parent
        ).exists()
    )
...
async def seq_docket(
    ...
    async def _scraper_(args, ind):
        ...
        while len(cases):
            ...
            if DktS.cost_limit and total_cost >= DktS.cost_limit:
                logging.info(
                    f"{DktS}: cost limit ($%.2f) {'reached' if total_cost==DktS.cost_limit else 'exceeded'}"
                    % DktS.cost_limit
                )
                break
            ...
            if case.get("download_attempts", 0) >= MAX_DOWNLOAD_ATTEMPTS:
                logging.info(
                    f"{DktS} skipping {case['ucid']}, exceeded max download attempts"
                )
                return

            exists = check_exists(
                subdir="html",
                pacer_path=DktS.dir.root.parent,
                ucid=case["ucid"],
                def_no=case.get("def_no"),
            )
            ...
            if exists and not docket_update:
                ...
                if DktS.verbose:
                    logging.info(
                        f"{DktS} <case: {case['ucid']}> already exists, skipping"
                    )
            else:
                ...
                if docket_path == PACER_ERROR_WRONG_CASE:
                    logging.info(
                        f"{DktS} PACER_ERROR_WRONG_CASE: served wrong case, pushing back onto end of queue"
                    )
    ...
    input_data = stools.parse_docket_input(
        query_results, docket_input, core_args["case_type"], core_args["court"], logging
    )
    ...
    df_cases["ucid"] = dtools.ucid(
        core_args["court"], df_cases["case_no"], allow_def_stub=True
    )

    prev_downloaded_map = map(
        lambda x: check_exists(
            subdir="html", ucid=x, pacer_path=core_args["court_dir"].root.parent
        ),
        df_cases.ucid,
    )
    ...
    if core_args["exclusions_path"]:
        exclude_df = get_excluded_cases(
            core_args["exclusions_path"], core_args["court"]
        )
    ...
    if docket_update:
        ...
        keepcols = [
            col
            for col in ("case_no", "ucid", "latest_date", "def_no")
            if col in df_cases.columns
        ]
    else:
        keepcols = [
            col for col in ("case_no", "ucid", "def_no") if col in df_cases.columns
        ]
    ...
    scrapers = [
        asyncio.create_task(_scraper_(args=core_args, ind=i))
        for i in range(core_args["n_workers"])
    ]
    ...
    if len(new_member_list_seen):
        new_member_list_seen = list(
            set(new_member_list_seen)
        )  # covering greg's bases (even if I missed something, this much redundancy can't hurt)
        ...
        logging.info(
            f"Added {len(new_member_list_seen)} new member cases to members list"
        )
...
async def seq_summary(core_args, summary_input):
    ...
    async def _scraper_(args, ind):
        ...
        SS = SummaryScraper(core_args={**core_args, "ind": ind})
        ...
        while len(cases):
            ...
            # Check if case_limit reached
            if core_args["case_limit"] and (
                len(results["success"]) >= core_args["case_limit"]
            ):
            ...
            exists = check_exists(
                subdir="summaries",
                pacer_path=core_args["court_dir"].root.parent,
                ucid=case["ucid"],
                def_no=case.get("def_no"),
            )
            if exists:
    ...
    if core_args["case_limit"]:
        ...
        logging.info(
            f"Applying case limit({core_args['case_limit']}), {len(cases):,} will be downloaded"
        )
...
async def seq_member(core_args, member_input):
    ...
    async def _scraper_(args, ind):
        ...
        MS = MemberScraper(core_args={**core_args, "ind": ind})
...
async def seq_document(
    ...
    dockets = [{"fpath": x} for x in new_dockets] or generate_dockets_list(
        document_input, core_args, skip_seen, all_docs
    )
    ...
    if core_args["case_limit"]:
        ...
        logging.info(
            f"Applying case limit({core_args['case_limit']}): {len(dockets)} case dockets will be included in Document Scraper"
        )
    logging.info(
        f"Document Limit of {document_limit:,} will be applied. Any individual case with more than {document_limit:,} documents will be skipped.\n"
    )
...
@click.option(
    "--mode",
    "-m",
    type=click.Choice(["all", *MODULES]),
    prompt=True,
    help="Choose to run all or a single scraper module",
    show_choices=True,
)
@click.option(
    "--n-workers",
...
@click.option("--court", "-c", prompt=True, help="Court abbrev. e.g. 'ilnd'")
@click.option(
    "--case-type",
    "-ct",
    default=None,
...
@click.option(
    "--runtime-start",
    "-rts",
    default=stools.PACER_HOURS_START,
    show_default=True,
    help="RunTime Start hour (in 24hrs, CDT)",
    type=click.IntRange(0, 23),
)
@click.option(
    "--runtime-end",
    "-rte",
    default=stools.PACER_HOURS_END,
    show_default=True,
    help="RunTime End hour (in 24hrs, CDT)",
    type=click.IntRange(0, 23),
)
@click.option(
    "--case-limit",
...
@click.option(
    "--docket-mem-list",
    "-mem",
    type=click.Choice(MEM_LIST_OPTS),
    default="never",
    show_default=True,
...
@click.option(
    "--document-att/--no-document-att",
    default=True,
    show_default=True,
    help="Document Scraper: whether or not to get document attachments",
)
@click.option(
    "--document-skip-seen/--no-document-skip-seen",
    is_flag=True,
    default=True,
    show_default=True,
...
def scraper(
    ...
    if case_limit == None:
        ...
        case_limit = click.prompt(
            "Case Limit: Please enter a limit on the # of cases to download (or enter 'false' for no limit)"
        )
    ...
    core_args = {
        ...
        "n_workers": n_workers,
        "exclusions_path": (
            Path(docket_exclusions).resolve() if docket_exclusions else None
        ),
    }
    ...
    if run_module["member"]:
        ...
        docket_results = asyncio.run(seq_member(core_args, member_input=member_input))
